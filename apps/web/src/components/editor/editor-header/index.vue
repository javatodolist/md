...
import { getCopyModePlatformInfo, usePostStore } from '@/stores/post'
...
import PlatformContentDialog from './PlatformContentDialog.vue'
...
const platformContentDialogVisible = ref(false)
...
// 复制到微信公众号
async function copy() {
  // 如果是 Markdown 源码，合并平台内容后复制
  if (copyMode.value === `md`) {
    const platformContentStore = usePlatformContentStore()
    const rawMd = editor.value?.state.doc.toString() || ``
    const mergedMd = platformContentStore.mergeContent(rawMd)
    await copyContent(mergedMd)
  ...
  setTimeout(() => {
    nextTick(async () => {
      ...
      if (postStore.currentPostId) {
        const pushPostId = postStore.currentPostId
        const { platform, accountName } = getCopyModePlatformInfo(copyMode.value)
        const record = {
          platform,
          accountId: `default`,
          accountName,
        ...
        postStore.addPushRecord(pushPostId, record)

        // 仅微信推送时监听真实账号信息
        if (copyMode.value === `txt`) {
          const onAccountInfo = (evt: Event) => {
            const { accountName: realName } = (evt as CustomEvent<{ accountName: string }>).detail
            if (realName) {
              const post = postStore.getPostById(pushPostId)
              const lastRecord = post?.pushRecords?.[post.pushRecords.length - 1]
              if (lastRecord && lastRecord.pushedAt === record.pushedAt) {
                lastRecord.accountName = realName
                lastRecord.accountId = realName
              }
            }
            window.removeEventListener(`pushAccountInfo`, onAccountInfo)
          }
          window.addEventListener(`pushAccountInfo`, onAccountInfo)
          setTimeout(() => window.removeEventListener(`pushAccountInfo`, onAccountInfo), 5000)
        }
      }
...
<template>
  ...
  >
    ...
    <div class="space-x-1 hidden md:flex">
      <Menubar class="menubar border-0">
        ...
        <FormatDropdown />
        <InsertDropdown />
        <StyleDropdown @open-platform-content="platformContentDialogVisible = true" />
        <ViewDropdown />
      ...
      <Menubar class="menubar border-0 p-0">
        <MenubarMenu>
          ...
          <MenubarContent align="start">
            ...
            <StyleDropdown :as-sub="true" @open-platform-content="platformContentDialogVisible = true" />
  ...
  <PlatformContentDialog :visible="platformContentDialogVisible" @close="platformContentDialogVisible = false" />
