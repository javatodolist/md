...
import DuplicatePushConfirmDialog from '../post-slider/DuplicatePushConfirmDialog.vue'
...
function post() {
  const checkedAccounts = form.value.accounts.filter(a => a.checked)
  ...
  if (currentId) {
    const accountsForCheck = checkedAccounts.map(a => ({
      platform: a.type,
      accountId: a.uid || a.displayName || `default`,
      accountName: a.displayName || a.title || a.type,
    }))
    const duplicates = postStore.findDuplicatePushes([currentId], accountsForCheck)
  ...
  // 无重复，直接发布
  form.value.accounts = checkedAccounts
  postTaskDialogVisible.value = true
  dialogVisible.value = false
}

function handleDuplicatePushAll() {
...
function handleDuplicateSkip() {
  // 过滤掉已推送过的平台
  const duplicatePlatformKeys = new Set<string>()
  for (const info of duplicateInfos.value) {
    for (const dup of info.duplicates) {
      duplicatePlatformKeys.add(`${dup.platform}_${dup.accountId}`)
    }
  }

  const filteredAccounts = form.value.accounts.filter((a) => {
    if (!a.checked)
      return false
    const key = `${a.type}_${a.uid || a.displayName || 'default'}`
    return !duplicatePlatformKeys.has(key)
  })

  if (filteredAccounts.length === 0) {
    toast.info(`所有选中平台均已推送过，无需重复推送`)
    return
  }

  form.value.accounts = filteredAccounts
  postTaskDialogVisible.value = true
  dialogVisible.value = false
}
