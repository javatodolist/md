...
/**
 * 清理 HTML 用于发布到外部平台（知乎、CSDN 等）
 * 移除仅在编辑器预览中有意义的元素和属性
 */
export function sanitizeForPublish(html: string): string {
  let result = html

  // 1. 移除所有 <style>...</style> 块（编辑器预览专用）
  result = result.replace(/<style[\s\S]*?<\/style>/gi, ``)

  // 2. 移除阅读时间 blockquote（编辑器专用统计信息）
  result = result.replace(
    /<blockquote class="md-blockquote">[\s\S]*?<\/blockquote>/gi,
    ``,
  )

  // 3. 移除 mac-sign 装饰（代码块 macOS 风格圆点）
  result = result.replace(/<span class="mac-sign"[\s\S]*?<\/span>/gi, ``)

  // 4. 解包 <section class="container">...</section> 外层容器
  result = result.replace(
    /^<section class="container">([\s\S]*)<\/section>$/i,
    `$1`,
  )

  // 5. 移除编辑器专用 class 属性
  result = result.replace(
    /\s+class="(p|h[1-6]|blockquote|ul|ol|listitem|hr|strong|em|codespan|td|th|tr|footnotes|figcaption|container|preview-table|md-blockquote|md-blockquote-p|md-figcaption)"/gi,
    ``,
  )

  // 6. 移除 data-heading 属性
  result = result.replace(/\s+data-heading="[^"]*"/gi, ``)

  // 7. 压缩标签间多余空白（多个换行/空格 → 单个换行）
