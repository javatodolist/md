...
import { usePlatformContentStore } from './platformContent'
...
export const useRenderStore = defineStore(`render`, () => {
  ...
  // 渲染内容
  const render = (content: string, options?: { platform?: string, skipMerge?: boolean }) => {
    if (!renderer) {
    ...
    if (!options?.skipMerge) {
      ...
      mergedContent = platformContentStore.mergeContent(
        content,
        options?.platform ?? platformContentStore.previewPlatform,
      )
    }
    ...
    // 渲染合并后的 Markdown
    const { html: baseHtml, readingTime: readingTimeResult } = renderMarkdown(mergedContent, renderer)

    // 统计信息基于原始内容（不含平台附加内容），避免统计偏差
