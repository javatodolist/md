...
export const useUIStore = defineStore(`ui`, () => {
  ...
  // 内容管理面板是否收拢（向左收起为竖条）
  const isPostSliderCollapsed = store.reactive(addPrefix(`is_post_slider_collapsed`), false)

  // 是否打开本地文件夹面板
  const isOpenFolderPanel = store.reactive(addPrefix(`is_open_folder_panel`), false)

  // 文件夹面板是否收拢（向左收起为竖条）
  const isFolderPanelCollapsed = store.reactive(addPrefix(`is_folder_panel_collapsed`), false)
